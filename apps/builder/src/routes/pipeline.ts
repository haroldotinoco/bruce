...
import { startBuilderPipeline } from '../services/pipeline.service.js';
...
pipelineRoutes.post('/', zValidator('json', bodySchema), async (c) => {
  ...
  try {
    const result = await startBuilderPipeline({
      accountId,
    ...
    logger.error({ error, accountId }, 'builder pipeline start failed');
