...
import { BUILDER_TASK_QUEUE } from './config.js';
...
export async function startWorker(): Promise<void> {
  ...
  const taskQueue = BUILDER_TASK_QUEUE;
  const namespace = process.env.TEMPORAL_NAMESPACE ?? 'default';

  try {
    logger.info({ address, taskQueue, namespace }, 'Starting Temporal worker (builder)');
    ...
    logger.info({ taskQueue }, 'Worker registered and listening (builder)');
    await worker.run();
  } catch (error) {
