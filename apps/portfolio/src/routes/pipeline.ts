...
import { startPortfolioPipeline } from '../services/pipeline.service.js';
...
pipelineRoutes.post('/', zValidator('json', bodySchema), async (c) => {
  ...
  try {
    const result = await startPortfolioPipeline({
      accountId,
    ...
    logger.error({ error, accountId }, 'portfolio pipeline start failed');
