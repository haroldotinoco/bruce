...
import { PORTFOLIO_TASK_QUEUE } from './config.js';
...
export async function startWorker(): Promise<void> {
  ...
  const taskQueue = PORTFOLIO_TASK_QUEUE;
  const namespace = process.env.TEMPORAL_NAMESPACE ?? 'default';

  try {
    logger.info({ address, taskQueue, namespace }, 'Starting Temporal worker (portfolio)');
    ...
    logger.info({ taskQueue }, 'Worker registered and listening (portfolio)');
    await worker.run();
  } catch (error) {
