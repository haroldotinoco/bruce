...
import { ADD_VENTURE_TASK_QUEUE } from './config.js';
...
export async function startWorker(): Promise<void> {
  ...
  const taskQueue = ADD_VENTURE_TASK_QUEUE;
  const namespace = process.env.TEMPORAL_NAMESPACE ?? 'default';

  try {
    logger.info({ address, taskQueue, namespace }, 'Starting Temporal worker (add-venture)');
