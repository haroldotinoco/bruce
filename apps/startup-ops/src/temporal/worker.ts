...
import { STARTUP_OPS_TASK_QUEUE } from './config.js';
...
export async function startWorker(): Promise<void> {
  ...
  const taskQueue = STARTUP_OPS_TASK_QUEUE;
  const namespace = process.env.TEMPORAL_NAMESPACE ?? 'default';

  try {
    logger.info({ address, taskQueue, namespace }, 'Starting Temporal worker (startup-ops)');
    ...
    logger.info({ taskQueue }, 'Worker registered and listening (startup-ops)');
    await worker.run();
  } catch (error) {
