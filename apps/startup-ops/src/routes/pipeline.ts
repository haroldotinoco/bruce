...
import { startStartupOpsPipeline } from '../services/pipeline.service.js';
...
pipelineRoutes.post('/', zValidator('json', bodySchema), async (c) => {
  ...
  try {
    const result = await startStartupOpsPipeline({
      accountId,
    ...
    logger.error({ error, accountId }, 'startup-ops pipeline start failed');
