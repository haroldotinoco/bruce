...
workflowsRoutes.post('/:run_id/force-handoff', async (c) => {
  ...
  try {
    ...
    const result = await forceHandoffFromWorkflow(
      accountId,
      runId,
      {
    ...
    if (err instanceof ForceHandoffError) {
      return c.json({ error: err.message }, err.status as 400 | 404 | 409 | 422);
    }
