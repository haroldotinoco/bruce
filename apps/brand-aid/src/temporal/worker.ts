...
import { BRAND_AID_TASK_QUEUE } from './config.js';
...
export async function startWorker(): Promise<void> {
  ...
  const taskQueue = BRAND_AID_TASK_QUEUE;
  const namespace = process.env.TEMPORAL_NAMESPACE ?? 'default';

  try {
    logger.info({ address, taskQueue, namespace }, 'Starting Temporal worker (brand-aid)');
    ...
    logger.info({ taskQueue }, 'Worker registered and listening (brand-aid)');
    await worker.run();
  } catch (error) {
