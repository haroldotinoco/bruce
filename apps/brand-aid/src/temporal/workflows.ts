...
const STEP_MARKET_ANALYST = 'market_analyst';

const TOP_LEVEL_STEPS = [
  {
    key: STEP_MARKET_ANALYST,
    label: 'Market analyst',
    icon: 'bar-chart-3',
    description: 'Run brand-aid market analyst agent.',
    agentIds: ['market-analyst'],
  },
];

export async function brandAidPipelineWorkflow(input: {
  account_id: string;
  venture_id: string;
  ...
  try {
    workflowState = { status: 'running', currentStep: STEP_MARKET_ANALYST };
    ...
    const result = await act.runMarketAnalystAgent({
      accountId: account_id,
      ventureId: venture_id,
      agentInput: agent_input,
      correlationId,
      observabilityRunId: obsRunId,
      observabilityStepKey: STEP_MARKET_ANALYST,
    });
    ...
    await act.obsUpdateStep({
      ...
      fields: {
        agent: { kind: 'text_short', value: 'market-analyst' },
      },
    });
