...
import { GTM_TASK_QUEUE } from './config.js';
...
export async function startWorker(): Promise<void> {
  ...
  const taskQueue = GTM_TASK_QUEUE;
  const namespace = process.env.TEMPORAL_NAMESPACE ?? 'default';

  try {
    logger.info({ address, taskQueue, namespace }, 'Starting Temporal worker (gtm)');
    ...
    logger.info({ taskQueue }, 'Worker registered and listening (gtm)');
    await worker.run();
  } catch (error) {
